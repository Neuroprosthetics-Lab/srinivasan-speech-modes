...
def compute_firing_rate_per_channel(data):
    ...
    for word in words:
        print(f'Number of {word} trials considered:', len(firing_rates[0][word]))
...
def get_significant_word_tuning_per_channel(firing_rates):
    ...
    for ch, rates in firing_rates.items():
        f_value, p_value = f_oneway(rates['DONOTHING'], rates[words[0]], rates[words[1]],)
                                            #    rates[words[2]], rates[words[3]], rates[words[4]], rates[words[5]]) # one-way anova
        # print(ch, f_value)
    ...
    # tuckey hsd: pairwise comparison of all the conditions
        tuckey_obj = tukey_hsd(rates[words[0]], rates[words[1]],)
                            #    rates[words[2]], rates[words[3]], 
                            #    rates[words[4]], rates[words[5]])
