# This script determines the channels tuned to different words.
import argparse
import scipy
import numpy as np
...
For t15,
    python ch_encoding_word.py --participant t15 --session word-loudness --nbins_before_onset 50 --nbins_after_onset 50 --savepath_data ../analyses_figures_data/t15/word-loudness/ch_encoding_word/ --savepath_fig ../analyses_figures/t15/word-loudness/ch_encoding_word/
For t16,
    python ch_encoding_word.py --participant t16 --session word-loudness --nbins_before_onset 50 --nbins_after_onset 50 --savepath_data ../analyses_figures_data/t16/word-loudness/ch_encoding_word/ --savepath_fig ../analyses_figures/t16/word-loudness/ch_encoding_word/
...
fig_fontsize = 20
scatter_size_scale = 20
color = (150/255, 54/255, 34/255)
...
def compute_firing_rate_per_channel(data):
    ...
    firing_rates = {}  # {channel: {word: [average firing rate per trial]}

    for ch in range(n_channels):
        firing_rates[ch] = {}
        for word in words:
                firing_rates[ch][word] = []
    ...
    for ind in speaking_inds:
        ...
        curr_word = data['cue'][ind].split(':')[1].strip()
        ...
        if threshcross[binned_delay_duration + start_ind - args.nbins_before_onset: binned_delay_duration + start_ind + args.nbins_after_onset, :].shape[0] == args.nbins_before_onset + args.nbins_after_onset:
            ...
            for ch in range(n_channels):
                firing_rates[ch][curr_word].append(mean_temp_thx[ch])

    for word in words:
        print(f'Number of {word} trials:', len(firing_rates[0][word]))
    
    return firing_rates


def get_significant_word_tuning_per_channel(firing_rates):

    # compute anova for each channel
    anova_results = {}  # {electrode: {word: p_value}}
    for ch, rates in firing_rates.items():
        f_value, p_value = f_oneway(rates[words[0]], rates[words[1]], rates[words[2]], rates[words[3]], rates[words[4]], rates[words[5]]) # one-way anova
        anova_results[ch] = [f_value, p_value]

    # for the significantly tuned channels get post hoc pairwise comparison
    n_significant_word_encoding = {} # {ch: value between 0 and combination(n_conditions, 2)}
    for ch, rates in firing_rates.items():
        tuckey_obj = tukey_hsd(rates[words[0]], rates[words[1]], rates[words[2]], rates[words[3]], rates[words[4]], rates[words[5]])

        # also get the word pairs that had different firing rates
        count = 0; word_pairs = []
        for i in range(len(tuckey_obj.pvalue)):
            for j in range(len(tuckey_obj.pvalue[i])):
                if i != j and tuckey_obj.pvalue[i][j] < alpha:
                    ...
                    if [j, i] not in word_pairs: # pairs appear twice
                        word_pairs.append([i,j])
        
        n_significant_word_encoding[ch] = [count/2, word_pairs] # key is channel, value is a tuple with count and list of loudness pairs, # each pair is counted twice, so divide count by 2

    return n_significant_word_encoding, anova_results
    ...
    for n, (ch_set, current_plotting_order) in enumerate(zip(ch_sets[args.participant], plotting_orders[args.participant])):
        ...
        # calculate row and col position
            ...
            if ch in mark_channels:
                ax[n].scatter(col, 7 - row, s = 17 * scatter_size_scale, facecolors='none', edgecolors='darkorange', linewidth = 2)
...
def plot_legend():
    ...
    txt = f'Number of word pairs with significantly different firing rates\n0 {n_pairs}'
...
if __name__ == "__main__":
    ...
    firing_rates = compute_firing_rate_per_channel(data) # {channel: {word: [average firing rate per trial]}

    # number of significant word tuning per channel
    print('Computing number of significant word tuning per channel ...')
    n_significant_word_encoding, anova_results = get_significant_word_tuning_per_channel(firing_rates) 
    ...
    with open(f'{args.savepath_data}{args.participant}_{args.session}_significant_channels_{formatted_datetime}.pkl', 'wb') as f:
        pkl.dump(n_significant_word_encoding, f)

    # number of channels per array tuned to words (# of filled circles)
    tot_n_ch_tuned_to_word = 0
    tot_n_ch = 0
    for i in range(len(ch_sets[args.participant])):
        ch_set = ch_sets[args.participant][i]
        tot_n_ch += len(ch_set)
        n_ch_tuned_to_word = 0
        for ch in ch_set:
            count_pairs = n_significant_word_encoding[ch][0]
            if count_pairs > 0:
                n_ch_tuned_to_word += 1

        tot_n_ch_tuned_to_word += n_ch_tuned_to_word
        
        print(f'% of word tuned channels in {ch_set_names[args.participant][i]}: {n_ch_tuned_to_word * 100/len(ch_set)}%')

    print(f'total % of word tuned channels: {tot_n_ch_tuned_to_word * 100/tot_n_ch}%')
    ...
    plot_significant_channels(n_significant_word_encoding, mark_channels[args.participant])
    plot_legend()
